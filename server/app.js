...
app.get('/api/expiredaccounts/', function (req, res) {
  ...
  if (UserEmail != null && APIkey != null && EXPdate != null) {
    connection.connect(function (err) {
      if (err) throw err;
      connection.query('UPDATE user_idpdetails LEFT JOIN client_details ON .universityid = universityid SET isActive = ?, expiryDate = ? WHERE email = ? AND APIkey = ?', [isactiveflag, EXPdate, UserEmail, APIkey], function (error, results, fields) {
        if (error) {
          console.log("error", error);
        } else {
          if (results != null) {
